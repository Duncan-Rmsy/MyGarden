...
/**
 * Pull a 16-day daily forecast (temperature min/max and precipitation) for a
 * coordinate pair. Returns `DailyWeather[]` with the same shape as `fetchHistory`;
 * rows with any non-finite reading are skipped identically.
 *
 * The forecast window starts today in local solar time (`timezone: 'auto'`), so
 * the current date may already exist in stored history. Deduplication is the
 * responsibility of the storage layer (saveForecast) — this function does not deduplicate.
 */
